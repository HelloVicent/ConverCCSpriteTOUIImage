...
//  Copyright 2013年 Vicent. All rights reserved.
//
...
-(id) init
{
	if ((self = [super init]))
	{
		...
		CCLabelTTF* label = [CCLabelTTF labelWithString:@"Hello Cocos2D!" fontName:@"Marker Felt" fontSize:CCRANDOM_0_1() * 40 + 30];
		...
		id rotate = [CCRotateBy actionWithDuration:600 angle:CCRANDOM_0_1() * 50000 + 10000];
...
-(void) addManyLabels
{
	...
	NSArray* strings = [NSArray arrayWithObjects:@"Hello!",
						@"Deploying.",
	...
	for (int i = 0; i < 50; i++)
	{
		...
		CCLabelTTF* label = [CCLabelTTF labelWithString:string fontName:font fontSize:CCRANDOM_0_1() * 16 + 8];
		...
		id rotate = [CCRotateBy actionWithDuration:600 angle:CCRANDOM_0_1() * 10000 + 5000];
		[label runAction:rotate];
	}
}
